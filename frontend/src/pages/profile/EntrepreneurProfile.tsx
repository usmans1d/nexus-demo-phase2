...
import { createCollaborationRequest, getMyRequests } from '../../data/collaborationRequests';
  ...
  React.useEffect(() => {
    if (isInvestor && currentUser && id) {
      getMyRequests().then(requests => {
        setHasRequestedCollaboration(requests.some((req: CollaborationRequest) => req.entrepreneurId === id));
      });
    }
